...
import 'package:ravola/Screens/google_login_screen.dart';
...
class TravelService {
  ...
  static Future<Map<String, String>> _solveChallenge(String url) async {
    ...
    final a =
        RegExp(r'a=toNumbers\("([a-f0-9]+)"\)').firstMatch(html)?.group(1) ??
        '';
    final b =
        RegExp(r'b=toNumbers\("([a-f0-9]+)"\)').firstMatch(html)?.group(1) ??
        '';
    final c =
        RegExp(r'c=toNumbers\("([a-f0-9]+)"\)').firstMatch(html)?.group(1) ??
        '';
  ...
  static Future<List<NearbyTravel>> getNearby({
    ...
    if (response.statusCode == 200) {
      ...
      return (data['travels'] as List)
          .map((e) => NearbyTravel.fromJson(e))
          .toList();
    }
    throw Exception(
      'Gagal mencari travel terdekat (status: ${response.statusCode})',
    );
  }
}
...
class _TravelScreenState extends State<TravelScreen> {
  ...
  void _showBookmarks() {
    final bookmarkedTravels = travels
                        ...
                        return ListTile(
                          ...
                          onTap: () {
                            ...
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (_) =>
                                    TravelDetailScreen(travelId: t.id),
                              ),
                            );
                          },
                        );
  ...
  void _showNearbyBottomSheet(List<NearbyTravel> results) {
    showModalBottomSheet(
      ...
      builder: (_) => _NearbyBottomSheet(
        results: results,
        onSelectTravel: (id) => Navigator.push(
          context,
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: Border.all(
                                  color: kGoldBright.withOpacity(0.5),
                                  width: 1,
                                ),
                              ),
                              child: const Icon(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [
                            BoxShadow(
                              color: (isNearby ? kGreen : kGold).withOpacity(
                                0.3,
                              ),
                              blurRadius: 12,
                              spreadRadius: 1,
...
class _ArchOrnamentPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final cx =
        size.width / 2; // center x (tidak dipakai size.width karena infinite)
    ...
    archPath.cubicTo(cx - w / 2, h * 0.04, cx - w * 0.12, 0, cx, 0);
    archPath.cubicTo(
      cx + w * 0.12,
      0,
      cx + w / 2,
            ...
            Padding(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 12),
                  const Text(
                    'Tidak ada travel dalam radius 50 km\ndari lokasi Anda saat ini.',
                    textAlign: TextAlign.center,
